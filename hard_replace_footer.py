...
def hard_replace_footer():
    ...
    # We will use regex to find `<footer...` up to `</footer>`
    pattern = re.compile(r'(<!-- FOOTER -->\s*)?<footer class="site-footer.*?</\s*footer\s*>', re.DOTALL | re.IGNORECASE)
    ...
    for filepath in html_files:
        ...
        if content != original_content:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"Replaced footer entirely in {os.path.basename(filepath)}")
        else:
